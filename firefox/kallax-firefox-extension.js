...
Parameters:
  chrome_runtime_id: String (this extension's id)
...
function login() {
  var theUrl = "";
  if (chrome.runtime.id.indexOf("@") > -1) {
    theUrl =
      "https://kallax.io/security_credentials/" +
      chrome.runtime.id.substr(0, chrome.runtime.id.indexOf("@"));
  } else {
    theUrl = "https://kallax.io/security_credentials/" + chrome.runtime.id;
  }
  console.log(theUrl);
...
function addToKallax(id) {
  ...
  const request =
    `https://api.kallax.io/api/collection/add/` +
    id.id +
    "?source=" +
    id.type.toLowerCase();
  const method = "POST";
  ...
  chrome.storage.sync.get(["gameIndex"], function (gameIndexResult) {
    ...
    try {
      chrome.storage.sync.get([title], function (instanceResult) {
        ...
        const request =
          `https://api.kallax.io/api/collection/delete/` + instance;
        const method = "DELETE";
        kallaxFetch(request, method)
          .then(function (res) {
